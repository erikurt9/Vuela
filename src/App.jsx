...
export default function VuelaRPAS() {
  ...
  const [savedProgress, setSavedProgress] = useState(null); // sesión de estudio sin terminar
  ...
  const mapRowToEntry = (row) => ({
    ...
    total: row.total,
    passed: row.passed,
    weakCat: row.weak_cat,
    weakPct: row.weak_pct,
    studyCategory: row.study_category,
    catStats: row.cat_stats,
    results: row.results,
    incomplete: row.incomplete,
    progress: row.progress,
  });

  // Inicializar sesión anónima de Supabase y cargar historial + progreso guardado
  useEffect(() => {
    (async () => {
      try {
        ...
        if (histRows) setHistory(histRows.map(mapRowToEntry));

        const { data: progRow, error: progErr } = await supabase
          .from("vuela_study_progress")
          .select("*")
          .eq("user_id", uid)
          .maybeSingle();
        if (progErr) throw progErr;
        if (progRow) setSavedProgress(progRow);
      } catch (e) {
        console.error("No se pudo conectar con Supabase:", e.message || e);
      }
      setHistLoaded(true);
    })();
  }, []);

  // Guardar resultado en historial (Supabase; máx 50 mostradas)
  const saveResult = useCallback(async (entry) => {
    setHistory(prev => [entry, ...prev].slice(0, 50));
    if (!userId) return;
    try {
      const { error } = await supabase.from("vuela_history").insert({
        user_id: userId,
        mode: entry.mode,
        date: entry.date,
        pct: entry.pct,
        ...
        weak_cat: entry.weakCat ?? null,
        weak_pct: entry.weakPct ?? null,
        study_category: entry.studyCategory ?? null,
        cat_stats: entry.catStats ?? null,
      ...
      console.error("No se pudo guardar el resultado en Supabase:", e.message || e);
    }
  }, [userId]);

  // Guardar avance de una sesión de Modo Estudio en curso (se sobreescribe, 1 fila por usuario)
  const saveProgress = useCallback(async ({ questions, current, selected, answered, studyResults, filterCat, category }) => {
    const localSnapshot = {
      questions, current, selected, answered,
      study_results: studyResults,
      filter_cat: filterCat,
      category,
    };
    setSavedProgress(localSnapshot);
    if (!userId) return;
    try {
      const { error } = await supabase.from("vuela_study_progress").upsert({
        user_id: userId,
        questions,
        current,
        selected,
      ...
      console.error("No se pudo guardar el avance en Supabase:", e.message || e);
    }
  }, [userId]);

  // Borrar el avance guardado (al terminar la sesión o al descartarla)
  const clearProgress = useCallback(async () => {
    setSavedProgress(null);
    if (!userId) return;
    try { await supabase.from("vuela_study_progress").delete().eq("user_id", userId); } catch {}
  }, [userId]);

  // Retomar la sesión de estudio guardada
  const resumeStudy = useCallback(() => {
    if (!savedProgress) return;
    setQuestions(savedProgress.questions || []);
    setCurrent(savedProgress.current || 0);
    setSelected(savedProgress.selected ?? null);
    setAnswered(savedProgress.answered || false);
    setStudyResults(savedProgress.study_results || []);
    setFilterCat(savedProgress.filter_cat || "Todas");
    setShowExp(false);
  ...
  // Guardar avance actual + registrar entrada visible en el historial, y salir
  const saveAndExitStudy = useCallback(() => {
    const total = studyResults.length;
    const ok = studyResults.filter(r => r.correct).length;
    ...
    const studyCategory = questions[0]?.category || filterCat;

    saveProgress({
      questions, current, selected, answered, studyResults, filterCat,
    ...
    if (total > 0) {
      saveResult({
        ...
        results: studyResults.map(r => ({ qId: questions[r.index]?.id, correct: r.correct, category: questions[r.index]?.category })),
      });
    }
    setScreen("home");
  }, [studyResults, questions, current, selected, answered, filterCat, saveProgress, saveResult]);
  ...
  const handleAnswer = (idx) => {
    ...
    if (mode === "study") {
      const updated = [...studyResults, { index: current, correct: idx === questions[current].correct }];
      setStudyResults(updated);
      saveProgress({
        questions, current, selected: idx, answered: true,
        studyResults: updated, filterCat,
        category: questions[0]?.category || filterCat,
      });
    }
  };
  ...
  const next = () => {
    ...
    setCurrent(nextIdx); setSelected(null); setAnswered(false); setShowExp(false);
    if (mode === "study") {
      saveProgress({
        questions, current: nextIdx, selected: null, answered: false,
        studyResults, filterCat,
        category: questions[0]?.category || filterCat,
      });
    }
    window.scrollTo(0, 0);
      ...
      <div className="container">
        <div className="main-grid">
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            {savedProgress && (
              <div className="card" style={{ animation: "fadeUp .6s .05s ease both", borderColor: "#10b981" }}>
                <div className="card-body" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                  <div>
                    <div className="card-header-title">📚 Tienes una sesión de estudio sin terminar</div>
                    <div className="card-header-sub">
                      Pregunta {Math.min((savedProgress.current || 0) + 1, savedProgress.questions?.length || 0)} de {savedProgress.questions?.length || 0}
                      {savedProgress.category ? ` · ${savedProgress.category}` : ""}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
                    <button className="btn-outline" onClick={clearProgress}>Descartar</button>
                    <button className="btn-primary" onClick={resumeStudy}>Continuar →</button>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="quiz-topbar">
          <div className="quiz-topbar-inner">
            <div className="quiz-meta">
              <button className="quiz-back" onClick={() => mode === "study" ? saveAndExitStudy() : setScreen("home")}>
                {mode === "study" ? "💾 Guardar y salir" : "← Salir"}
              </button>
        ...
        {histDetail && (
          <div className="hist-detail-overlay">
            ...
            <div className="hist-detail-content">
              {histDetail.incomplete && (
                <div className="weak-cat-banner" style={{ borderColor: "#065f46" }}>
                  <span className="weak-cat-icon">⏸</span>
                  <div>
                    <div className="weak-cat-title">Sesión en pausa · {histDetail.progress}</div>
                    <div className="weak-cat-sub">Puedes continuar exactamente donde la dejaste.</div>
                  </div>
                  {savedProgress ? (
                    <button onClick={() => { setHistDetail(null); resumeStudy(); }}
                      style={{ marginLeft: "auto", fontSize: 12, color: "#34d399", background: "transparent", border: "1px solid #065f46", borderRadius: 8, padding: "7px 14px", cursor: "pointer", whiteSpace: "nowrap" }}>
                      Continuar →
                    </button>
                  ) : (
                    <span style={{ marginLeft: "auto", fontSize: 11, color: "#64748b" }}>Ya no disponible</span>
                  )}
                </div>
              )}
            ...
            <>
                  ...
                  <div key={h.id} className="hist-item" onClick={() => setHistDetail(h)}>
                    ...
                    <div className="hist-item-info">
                      <div className="hist-item-title">
                        ...
                        {h.incomplete && <span style={{ marginLeft: 8, fontSize: 11, color: "#fbbf24" }}>⏸ En pausa · {h.progress}</span>}
                      </div>
              ...
              <div style={{ textAlign: "center", marginTop: 24 }}>
                <button onClick={async () => {
                  ...
                  setHistory([]);
                  if (userId) {
                    try { await supabase.from("vuela_history").delete().eq("user_id", userId); } catch {}
                  }
